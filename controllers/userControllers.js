...
const registerUser = async (req, res, next) => {
   try {
      ...
      if((password.trim()).length < 6) {
         return next(new HttpError("Password should contain atleast 6 character", 422))
      }
      ...
      res.status(201).json(`New User ${newUser.email} registered`)
        ...
        console.error(error);
        return next(new HttpError("User Registraion failed", 422))
   }
}
...
const loginUser = async (req, res, next) => {
    try {
       ...
       if(!email || !password) {
           return next(new HttpError("Fill in alasl fields", 422))
       } 
       const newEmail = email.toLowerCase();
...
// =================== EDIT USER DETAILS (from picture) ==============
...
const editUser = async (req, res, next) => {
   try {
      ...
      if(emailExist && (emailExist._id != req.user.id)) {
         return next(new HttpError("Email already exust", 422))
      }
